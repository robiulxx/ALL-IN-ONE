...
# ----------------------------
def send_code_thread(bot, user_id, data):
    try:
        api_id = data["api_id"]
...
def sign_in_thread(bot, user_id, data):
    try:
        session_type = data["session_type"]
        ...
        if session_type == "pyrogram":
            phone_code_hash = getattr(data.get("sent_code_info"), "phone_code_hash", None)
...
def check_password_thread(bot, user_id, data):
    try:
        session_type = data["session_type"]
        temp_client = data["temp_client"]
...
def register(bot, custom_command_handler, COMMAND_PREFIXES):
    ...
    # ----------------------------
    # Text input handler (user-specific)
    # ----------------------------
    @bot.message_handler(func=lambda message: user_steps.get(message.from_user.id))
    def handle_text(message):
        ...
        if step == "waiting_for_phone_number":
            ...
            Thread(target=send_code_thread, args=(bot, user_id, user_temp_data[user_id])).start()
            return

        if step == "waiting_for_code":
            user_temp_data[user_id]["otp_code"] = text
            user_steps[user_id] = "waiting_for_password"
            # Start thread to sign in
            Thread(target=sign_in_thread, args=(bot, user_id, user_temp_data[user_id])).start()
            return

        if step == "waiting_for_password":
            user_temp_data[user_id]["password"] = text
            # Start thread to check password
            Thread(target=check_password_thread, args=(bot, user_id, user_temp_data[user_id])).start()
            return
